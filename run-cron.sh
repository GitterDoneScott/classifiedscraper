...
#echo "0 */1 * * * cd /usr/src/app && /usr/local/bin/scrapy list| xargs -n 1 /usr/local/bin/scrapy crawl > /proc/1/fd/1 2>/proc/1/fd/2" >> ${CRON_FILE}
echo "0 */1 * * * cd /usr/src/app && echo 'facebook pinkbike ebay craigslist jeffmartin' | xargs -n 1 /usr/local/bin/scrapy crawl > /proc/1/fd/1 2>/proc/1/fd/2" >> ${CRON_FILE}
